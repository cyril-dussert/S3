...
class Config {
    ...
    _getConfig() {
        let config;
        //Check if a full config file is provided through environment variables (JSON object)
        try {
            if (process.env.S3_CONFIG_JSON !== undefined) {
                config = JSON.parse(process.env.S3_CONFIG_JSON);
            }
            else {
        ...
        this.port = 8000;
        //Check if port configuration is provided through environment variables (Integer)
        if (process.env.S3PORT !== undefined) {
            assert(Number.isInteger(process.env.S3PORT) && process.env.S3PORT >0, 
                    'bad environment (S3PORT): port must be a positive integer');
            this.port = process.env.S3PORT;
        }
        ...
        //Check if regions configuration are provided through environment variables (JSON object)
        const regions = process.env.S3REGIONS ?
          JSON.parse(process.env.S3REGIONS) : config.regions;
        assert(typeof regions === 'object',
               'bad config: the list of regions is mandatory');
        assert(Object.keys(regions).every(
               r => typeof r === 'string' && regions[r]
               instanceof Array
               && regions[r].every(e => typeof e === 'string')),
